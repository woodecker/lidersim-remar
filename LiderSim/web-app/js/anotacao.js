...
function _modal_edit(tr){
    var url = location.origin + '/lidersim/anotacao/returnInstance/' + $(tr).attr('data-id');
...
function _delete() {
    ...
    if(list_id_delete.length==1){
        url = location.origin + '/lidersim/anotacao/delete/' + list_id_delete[0];
        data = {_method: 'DELETE'};
        ...
        for(var i=0;i<list_id_delete.length;i++){
            url = location.origin + '/lidersim/anotacao/delete/' + list_id_delete[i];
            data = {_method: 'DELETE'};
...
function _submit() {
    ...
    if($("input[type=checkbox]:checked").size() < 1) {
        ...
        $.ajax({
            type: "POST",
            traditional: true,
            url: "/lidersim/anotacao/exportAnotacoes",
